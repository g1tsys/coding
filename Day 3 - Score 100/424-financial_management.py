...
你需要在以下约束下，选择最多2个产品进行投资，使得：
- 总投资额 ≤ N
- 总风险值 ≤ X（风险值 = 各产品投资额 × 对应风险值）
- 总回报（投资额 × 回报率）最大化

输出：
- 一个长度为 `m` 的整数数组，表示每个产品的最优投资额（按产品顺序输出）

输入格式：
- 第一行：`m`（产品数）、`N`（总投资额）、`X`（可接受总风险）
- 第二行：`m` 个整数，表示各产品的回报率
- 第三行：`m` 个整数，表示各产品的风险值
- 第四行：`m` 个整数，表示各产品的最大投资额

输出格式：
- 一个长度为 `m` 的整数序列，表示每个产品的最优投资额
...
- 第二行：`m` 个整数，表示各产品的投资回报率（1–60）
- 第三行：`m` 个整数，表示各产品的风险值（1–100）
- 第四行：`m` 个整数，表示各产品的最大投资额度（1–10000）

输出格式：
- 一个长度为 `m` 的整数序列，表示每个产品的最优投资额（按产品顺序输出）
